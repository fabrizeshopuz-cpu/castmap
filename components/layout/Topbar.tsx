    ...
    <header className="flex min-h-24 items-center justify-between gap-5 border-b border-white/10 bg-black/35 px-7 py-5 max-xl:flex-col max-xl:items-stretch">
      ...
      <div className="flex flex-wrap items-center justify-end gap-3 max-xl:justify-start">
        <label className="relative w-[420px] max-w-full">
          ...
          <input className="h-11 w-full rounded-xl border border-white/10 bg-white/[0.04] pl-11 pr-4 text-sm text-white outline-none placeholder:text-castMuted" placeholder="Qidirish..." />
        </label>
        <button className="grid h-11 w-11 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-castGold" type="button" aria-label="Bildirishnoma">
          <Bell className="h-5 w-5" />
        </button>
        <button className="grid h-11 w-11 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-castMuted" type="button" aria-label="Yordam">
          <HelpCircle className="h-5 w-5" />
        </button>
        <button
          className="relative flex h-11 w-[92px] items-center justify-between rounded-xl border border-white/10 bg-white/[0.04] px-3 text-castGold"
          type="button"
        ...
        >
          <span className={`absolute top-1 h-9 w-9 rounded-lg bg-gradient-to-br from-[#FFE18A] to-castDeepGold transition-all duration-200 ${theme === "light" ? "left-1" : "left-[51px]"}`} />
        ...
        <button className="flex h-11 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm text-white" type="button">
